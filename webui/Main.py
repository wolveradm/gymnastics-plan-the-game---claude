...
def open_task_folder(task_id):
    try:
        # task_id 应始终是服务端生成的 UUID。这里先做格式校验，避免异常値
            ...
            if llm_provider == "aihubmix":
                ...
                with llm_helper:
                    tips = """
                            ...
                            - **能力完整**: 文本、图片生成、视频生成、TTS、STT、向量嵌入、Rerank，多模态场景全搭定
...
with middle_panel:
    with st.container(border=True):
        ...
        # Coverr 库 99% 是 16:9 横屏,默认杚屏会让画面被大量黑边包围。
    ...
    with st.container(border=True):
        ...
        # 无配音模式会生成静音占位音频，不展示试听按鈕，避免用户误以为需要测试声音。
        if (
            friendly_names
        ...
        # 这样用户如果同时使用 MiMo 生成文案和语音，只需要维护一份密鑢。
...
with right_panel:
    ...
    with st.expander(tr("Click to show API Key management"), expanded=False):
        ...
        with col3:
            ...
            # 老用户的 config.toml 不一定包含,这里先居底初始化为空列表,
